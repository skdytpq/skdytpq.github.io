...
$MSE(\theta_0,\theta_1)$ = $\sum_i^n (y_i - \theta_0x_{i1}-\theta_1x_{i2})^2 =(\sum_i^nx^2_{i1})\theta_0^2 + (\sum_i^2x_{i2}^2)\theta_1^22(\sum_i^nx_{i1}x_{i2})\theta_0\theta_1 -2(\sum_i^ny_ix_{i1})\theta_0-2(\sum_i^2y_ix_{i2})\theta_1 + \sum_i^ny_i^2$  로 풀어쓸 수 있는데 
...
식으로 나타내면 <br>$$J(\theta) = MSE(\theta) + \alpha\sum_i^n|\theta_i|$$에서 볼 수 잇듯 규제항에 절대값을 씌운 것이다.
