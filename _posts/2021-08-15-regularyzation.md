...
<br>$MSE_{(training)} = (Y - \hat{Y})^2$ 간단한 수식으로 이렇게 나타낼 수 있는데, 이것에 대한 기댓값을 전개한다면 $Expected MSE = E[(Y-\hat{Y})^2\vert X]$ 의 형태가 된다.
...
= $(\sum_i^n x^2_{i1}) \theta_0^2 + (\sum_i^2 x_{i2}^2) \theta_1^2$ 
